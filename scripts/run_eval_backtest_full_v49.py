...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"[eval-backtest-v49] output={out_root}")
    print(f"[eval-backtest-v49] windows={len(windows)} targets={len(targets)} agents={agents}")

    if not ns.skip_real_pipeline:
        for i, as_of in enumerate(windows, 1):
            print(f"[eval-backtest-v49] real-pipeline {i}/{len(windows)} as_of={as_of}")
        ...
        print("[eval-backtest-v49] skip-real-pipeline enabled; rebuilding standardized evaluation files only.")

    print("[eval-backtest-v49] building standardized evaluation CSV/XLSX ...")
    ...
    print(f"[eval-backtest-v49] DONE output_root={final_root}")
    print(f"[eval-backtest-v49] open: {final_root / f'eval_history_outputs_{ns.frequency}_{ns.start}_to_{ns.end}.xlsx'}")
    return 0
