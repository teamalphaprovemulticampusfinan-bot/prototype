...
def main(argv: list[str] | None = None) -> int:
    ...
    run_id = _safe_name(ns.run_id or f"{ns.frequency}_{ns.start}_{ns.end}_v39")
    ...
    # No fixed Buy/Hold/Sell threshold. Directional argmax follows larger Buy vs Sell posterior mass and only holds exact ties/missing evidence.
    ...
    print(f"[eval-backtest-v39] output={out_root}")
    print(f"[eval-backtest-v39] windows={len(windows)} targets={len(targets)} agents={agents}")

    if not ns.skip_real_pipeline:
        for i, as_of in enumerate(windows, 1):
            print(f"[eval-backtest-v39] real-pipeline {i}/{len(windows)} as_of={as_of}")
        ...
        print("[eval-backtest-v39] skip-real-pipeline enabled; rebuilding standardized evaluation files only.")

    print("[eval-backtest-v39] building standardized evaluation CSV/XLSX ...")
    ...
    print(f"[eval-backtest-v39] DONE output_root={final_root}")
    print(f"[eval-backtest-v39] open: {final_root / f'eval_history_outputs_{ns.frequency}_{ns.start}_to_{ns.end}.xlsx'}")
    return 0
