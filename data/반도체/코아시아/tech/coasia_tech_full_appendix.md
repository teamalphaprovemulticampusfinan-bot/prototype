# 코아시아 Tech Agent Full Appendix

- **생성 시각:** 2026-05-26T21:03:28
...
- **산출물 상태:** 확인됨: `data/반도체/코아시아/tech/coasia_tech_high_quality_report.md`
...
| 활용 및 확장 산업 | 활용 산업 | 3 / 5 (확장성) | # 코아시아 Tech Local Proxy Evidence; 직접근거 3건, 공식/DART/IR/특허 근거 0건, 정량화 지표 3건 | 중 | data\반도체\코아시아\tech\source\coasia_tech_local_proxy_evidence.m... |
...
- **산출물 상태:** 확인됨: `data/반도체/코아시아/tech/coasia_tech_to_value_inputs.json`
| 항목 | 값 | 해석 |
|---|---:|---|
...
- **산출물 상태:** 확인됨: `data/반도체/코아시아/tech/coasia_tech_patent_evidence.json`
| 원천 신호 | 값 |
|---|---:|
| 정규화 특허 텍스트 레코드 | 2,106건 |
| 회사 출원인/권리자 매칭 | 1,885건 |
| 등록 특허 | 229건 |
| 존속 가능 특허 | 1,820건 |
| 최근 5년 특허 | 1,560건 |
| IPC/CPC 다양성 | 7개 |
...
|---|---|
| 발광 다이오드 패키지 | 등록 |
| 방열 기능이 구비된 엘이디 칩 스케일 패키지 및 방열 기능이 구비된 엘이디 패키지 | 등록 |
| 고화소 소형 매크로 광학계 | 등록 |
| TOF 카메라용 광학계 | 등록 |
| 어라운드뷰 제공 방법 및 장치 | 등록 |
## Step 4. Patent KMeans clustering
- **산출물 상태:** 확인됨: `data/반도체/코아시아/tech/tech_ml_signal.json`
| 항목 | 값 |
|---|---:|
| 모델 | tfidf_kmeans |
| 정규화 특허 텍스트 레코드 | 30건 |
...
|---:|---:|---|---|
| 2 | 13건 | 데이터를, 작업, 그래픽, 영상, 쉐이딩, 웨어러블, tv | 기술 텍스트 유사도 기반 클러스터 |
| 0 | 12건 | 렌즈, 굴절력을, 굴절력을 가지는, 가지는, 정의 굴절력을, 정의, 부의 굴절력을 | 기술 텍스트 유사도 기반 클러스터 |
| 1 | 5건 | 발광, 다이오드, 발광 다이오드, 패키지, 발광부, 전극 | 기술 텍스트 유사도 기반 클러스터 |
...
- **산출물 상태:** 확인됨: `data/반도체/코아시아/tech/tech_ip_ml.json`
### 9-1. Tech/IP Strength Index
| 항목 | 값 | 해석 |
|---|---:|---|
| Tech/IP Strength Index | 75.49/100 | 특허 등록률·존속성·최근성·IPC/CPC 다양성·핵심기술 키워드 밀도 점수를 통합한 IP 강도 지표입니다. |
| Reference Universe Percentile | 50.00% | reference universe 안에서의 상대적 기술/IP 위치입니다. |
| Patent Momentum Score | 87.07/100 | 최근 특허 활동과 기술 포트폴리오 지속성을 반영합니다. |
...
### 9-4. 해석
- coasia의 Tech/IP Strength Index는 75.49/100, percentile 50.0로 산출되어 평균 이상 IP 포지션으로 해석됩니다. NMF 기준 주된 기술 주제는 '주요 topic 확인 제한'입니다. 단, 이 지표는 기술/IP 포트폴리오 상대 강도 신호이며 고객 채택·양산·매출 전환·FCF 개선 근거를 대체하지 않습니다.
...
### 10-5. 해석
- 코아시아의 Technology Differentiation Score는 peer 대비 기술/IP 포트폴리오 차별성 신호입니다.
...
| 구분 | 경로 | 상태 |
|---|---|---|
| chair_summary | `data/반도체/코아시아/tech/tech_chair_summary.json` | 확인됨 |
| tech_packet | `data/반도체/코아시아/tech/tech.json` | 확인됨 |
| high_quality_md | `data/반도체/코아시아/tech/coasia_tech_high_quality_report.md` | 확인됨 |
| tech_to_value | `data/반도체/코아시아/tech/coasia_tech_to_value_inputs.json` | 확인됨 |
| patent_json | `data/반도체/코아시아/tech/coasia_tech_patent_evidence.json` | 확인됨 |
| ml_signal | `data/반도체/코아시아/tech/tech_ml_signal.json` | 확인됨 |
...
| tech_ip_ml | `data/반도체/코아시아/tech/tech_ip_ml.json` | 확인됨 |
