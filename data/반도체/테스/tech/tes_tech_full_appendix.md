# 테스 Tech Agent Full Appendix

- **생성 시각:** 2026-05-20T22:03:28
...
- **산출물 상태:** 확인됨: `data/반도체/테스/tech/tes_tech_high_quality_report.md`
...
| 활용 및 확장 산업 | 활용 산업 | 3 / 5 (확장성) | # 테스 Tech Local Proxy Evidence; 직접근거 3건, 공식/DART/IR/특허 근거 0건, 정량화 지표 3건 | 중 | data\반도체\테스\tech\source\tes_tech_local_proxy_evidence.md |
...
- **산출물 상태:** 확인됨: `data/반도체/테스/tech/tes_tech_to_value_inputs.json`
| 항목 | 값 | 해석 |
|---|---:|---|
...
- **산출물 상태:** 확인됨: `data/반도체/테스/tech/tes_tech_patent_evidence.json`
| 원천 신호 | 값 |
|---|---:|
| 정규화 특허 텍스트 레코드 | 10,146건 |
| 회사 출원인/권리자 매칭 | 5,282건 |
| 등록 특허 | 1,729건 |
| 존속 가능 특허 | 1,991건 |
| 최근 5년 특허 | 53건 |
| IPC/CPC 다양성 | 17개 |
...
| 대표 특허 예시 | 상태/연도 |
|---|---|
| 합성 그래핀의 도핑을 포함하는, 큰-면적/웨이퍼 스케일 CMOS-양립 가능 2D-재료 인터칼레이션 도핑 툴, 프로세스, 및 방법 | 등록 |
| 적층 칩 및 적층 칩의 제조 방법 | 등록 |
| 기판처리장치 및 기판처리방법 | 등록 |
| 플레이트 기판 타입의 연속공정 방식 태양광 모듈의 전기적 특성 시험장치 | 등록 |
| 기판지지유닛 | 등록 |
## Step 4. Patent KMeans clustering
- **산출물 상태:** 확인됨: `data/반도체/테스/tech/tech_ml_signal.json`
| 항목 | 값 |
|---|---:|
| 모델 | tfidf_kmeans |
| 정규화 특허 텍스트 레코드 | 382건 |
...
|---:|---:|---|---|
| 6 | 131건 | 복수의, 프로브, 적어도, 기판 | 기술 텍스트 유사도 기반 클러스터 |
| 0 | 55건 | 배터리, 제어, 변속, 충전 | 기술 텍스트 유사도 기반 클러스터 |
| 5 | 44건 | 연료, 엔진, 진단, 제어, 차량의, 밸브 | 기술 텍스트 유사도 기반 클러스터 |
| 2 | 42건 | 테스트, 데이터, 테스트 대상, 디바이스, 대상 | 기술 텍스트 유사도 기반 클러스터 |
| 4 | 31건 | 기판을, 대한, 기판의, 기판, 챔버 | 기술 텍스트 유사도 기반 클러스터 |
| 7 | 29건 | 신호, 지연, 회로, 출력, 타이밍, 복수의, 클럭, 신호를 | 기술 텍스트 유사도 기반 클러스터 |
| 3 | 27건 | 시험, 피시험, 메모리, 불량, 패턴, 반도체, 복수의, 시험 장치 | 기술 텍스트 유사도 기반 클러스터 |
| 1 | 23건 | 편조, 기어, 포함할, 17/00 | 기술 텍스트 유사도 기반 클러스터 |
...
- **산출물 상태:** 확인됨: `data/반도체/테스/tech/tech_ip_ml.json`
### 9-1. Tech/IP Strength Index
| 항목 | 값 | 해석 |
|---|---:|---|
| Tech/IP Strength Index | 41.56/100 | 특허 등록률·존속성·최근성·IPC/CPC 다양성·핵심기술 키워드 밀도 점수를 통합한 IP 강도 지표입니다. |
| Reference Universe Percentile | 50.00% | reference universe 안에서의 상대적 기술/IP 위치입니다. |
| Patent Momentum Score | 0.75/100 | 최근 특허 활동과 기술 포트폴리오 지속성을 반영합니다. |
...
### 9-4. 해석
- tes의 Tech/IP Strength Index는 41.56/100, percentile 50.0로 산출되어 근거 보강 필요 IP 포지션으로 해석됩니다. NMF 기준 주된 기술 주제는 '주요 topic 확인 제한'입니다. 단, 이 지표는 기술/IP 포트폴리오 상대 강도 신호이며 고객 채택·양산·매출 전환·FCF 개선 근거를 대체하지 않습니다.
...
### 10-5. 해석
- 테스의 Technology Differentiation Score는 peer 대비 기술/IP 포트폴리오 차별성 신호입니다.
...
| 구분 | 경로 | 상태 |
|---|---|---|
| chair_summary | `data/반도체/테스/tech/tech_chair_summary.json` | 확인됨 |
| tech_packet | `data/반도체/테스/tech/tech.json` | 확인됨 |
| high_quality_md | `data/반도체/테스/tech/tes_tech_high_quality_report.md` | 확인됨 |
| tech_to_value | `data/반도체/테스/tech/tes_tech_to_value_inputs.json` | 확인됨 |
| patent_json | `data/반도체/테스/tech/tes_tech_patent_evidence.json` | 확인됨 |
| ml_signal | `data/반도체/테스/tech/tech_ml_signal.json` | 확인됨 |
...
| tech_ip_ml | `data/반도체/테스/tech/tech_ip_ml.json` | 확인됨 |
