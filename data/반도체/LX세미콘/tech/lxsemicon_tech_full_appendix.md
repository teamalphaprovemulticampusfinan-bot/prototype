...
| 활용 및 확장 산업 | 활용 산업 | 3 / 5 (확장성) | # LX세미콘 Tech Local Proxy Evidence; 직접근거 3건, 공식/DART/IR/특허 근거 0건, 정량화 지표 3건 | 중 | data\반도체\LX세미콘\tech\source\lxsemicon_tech_local_proxy_eviden... |
...
| 대표 특허 예시 | 상태/연도 |
|---|---|
| 전력반도체 소자 및 이를 포함하는 전력변환 장치와 전력반도체 소자의 제조방법 | 공개 |
| 디스플레이 구동 장치 및 디스플레이 구동 방법 | 등록 |
| 타이밍 컨트롤러, 이를 포함하는 디스플레이 구동장치 및 타이밍 컨트롤러를 구동하는 방법 | 등록 |
| 직류-직류 변환기 및 직류-직류 변환기를 포함하는 디스플레이 구동장치 | 등록 |
| 디스플레이 장치의 타이밍 컨트롤러 | 등록 |
...
|---:|---:|---|---|
| 0 | 49건 | 디스플레이, 데이터, 출력, 구동, 디스플레이 구동 | 기술 텍스트 유사도 기반 클러스터 |
| 3 | 22건 | 터치, 실시예는, 구동, 센싱, 터치센싱장치, 디스플레이, 터치 센싱 | 기술 텍스트 유사도 기반 클러스터 |
| 2 | 11건 | 블록의, 현재, 머지, 영상, 현재 블록의, 복호화 | 기술 텍스트 유사도 기반 클러스터 |
| 1 | 8건 | 화면, 화면 예측, 예측, 화면 예측에, 예측에, 예측에 관한 | 기술 텍스트 유사도 기반 클러스터 |
...
| 항목 | 값 | 해석 |
|---|---:|---|
| Tech/IP Strength Index | 32.08/100 | 특허 등록률·존속성·최근성·IPC/CPC 다양성·핵심기술 키워드 밀도 점수를 통합한 IP 강도 지표입니다. |
| Reference Universe Percentile | 50.00% | reference universe 안에서의 상대적 기술/IP 위치입니다. |
| Patent Momentum Score | 3.62/100 | 최근 특허 활동과 기술 포트폴리오 지속성을 반영합니다. |
...
### 9-4. 해석
- lxsemicon의 Tech/IP Strength Index는 32.08/100, percentile 50.0로 산출되어 근거 보강 필요 IP 포지션으로 해석됩니다. NMF 기준 주된 기술 주제는 '주요 topic 확인 제한'입니다. 단, 이 지표는 기술/IP 포트폴리오 상대 강도 신호이며 고객 채택·양산·매출 전환·FCF 개선 근거를 대체하지 않습니다.
