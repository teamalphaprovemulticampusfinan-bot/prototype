# 제주반도체 Tech Agent Full Appendix

- **생성 시각:** 2026-05-26T20:09:58
...
- **산출물 상태:** 확인됨: `data/반도체/제주반도체/tech/jeju_semicon_tech_high_quality_report.md`
...
| 핵심 제품/서비스 | 제품명/서비스명 | 3 / 5 (수익성 기여) | # 제주반도체 Tech Local Proxy Evidence; 직접근거 6건, 공식/DART/IR/특허 근거 0건, 정량화 지표 3건 | 중 | data\반도체\제주반도체\tech\source\jeju_semicon_tech_local_proxy_evi... |
...
| 활용 및 확장 산업 | 활용 산업 | 3 / 5 (확장성) | # 제주반도체 Tech Local Proxy Evidence; 직접근거 3건, 공식/DART/IR/특허 근거 0건, 정량화 지표 3건 | 중 | data\반도체\제주반도체\tech\source\jeju_semicon_tech_local_proxy_evi... |
...
- **산출물 상태:** 확인됨: `data/반도체/제주반도체/tech/jeju_semicon_tech_to_value_inputs.json`
| 항목 | 값 | 해석 |
|---|---:|---|
...
- **산출물 상태:** 확인됨: `data/반도체/제주반도체/tech/jeju_semicon_tech_patent_evidence.json`
| 원천 신호 | 값 |
|---|---:|
...
|---|---|
| 반도체 디바이스의 파워업 회로 | 등록 |
| 크로스 토크를 감소시키는 이미지 센서의 소자 분리 구조체및 그 제조방법 | 소멸 |
| 다양한 형태의 디스플레이 장치에 적용 가능한 반도체 메모리 디바이스 | 등록 |
| 공정조건에 안정적인 온도 보상형 셀프 리프레쉬 제어 회로 | 등록 |
| 반도체 장치 및 그의 레이아웃 배치방법 | 소멸 |
## Step 4. Patent KMeans clustering
- **산출물 상태:** 확인됨: `data/반도체/제주반도체/tech/tech_ml_signal.json`
| 항목 | 값 |
|---|---:|
| 모델 | tfidf_kmeans |
| 정규화 특허 텍스트 레코드 | 26건 |
...
|---:|---:|---|---|
| 0 | 13건 | 메모리, 반도체, 반도체 메모리, 서브, 발명의 반도체, 액티브 | 기술 텍스트 유사도 기반 클러스터 |
| 2 | 8건 | 전원, 비트, 디벨로프, 외부 전원, 비트 라인, 전압을 | 기술 텍스트 유사도 기반 클러스터 |
| 1 | 5건 | 리프레쉬, 셀프 리프레쉬, 셀프, 파워업, 분리, 동작을, 디램 | 기술 텍스트 유사도 기반 클러스터 |
...
- **산출물 상태:** 확인됨: `data/반도체/제주반도체/tech/tech_ip_ml.json`
### 9-1. Tech/IP Strength Index
| 항목 | 값 | 해석 |
|---|---:|---|
| Tech/IP Strength Index | 55.86/100 | 특허 등록률·존속성·최근성·IPC/CPC 다양성·핵심기술 키워드 밀도 점수를 통합한 IP 강도 지표입니다. |
| Reference Universe Percentile | 50.00% | reference universe 안에서의 상대적 기술/IP 위치입니다. |
| Patent Momentum Score | 89.78/100 | 최근 특허 활동과 기술 포트폴리오 지속성을 반영합니다. |
...
### 9-4. 해석
- jeju_semicon의 Tech/IP Strength Index는 55.86/100, percentile 50.0로 산출되어 중간 수준 IP 포지션으로 해석됩니다. NMF 기준 주된 기술 주제는 '주요 topic 확인 제한'입니다. 단, 이 지표는 기술/IP 포트폴리오 상대 강도 신호이며 고객 채택·양산·매출 전환·FCF 개선 근거를 대체하지 않습니다.
...
### 10-5. 해석
- 제주반도체의 Technology Differentiation Score는 peer 대비 기술/IP 포트폴리오 차별성 신호입니다.
...
| 구분 | 경로 | 상태 |
|---|---|---|
| chair_summary | `data/반도체/제주반도체/tech/tech_chair_summary.json` | 확인됨 |
| tech_packet | `data/반도체/제주반도체/tech/tech.json` | 확인됨 |
| high_quality_md | `data/반도체/제주반도체/tech/jeju_semicon_tech_high_quality_report.md` | 확인됨 |
| tech_to_value | `data/반도체/제주반도체/tech/jeju_semicon_tech_to_value_inputs.json` | 확인됨 |
| patent_json | `data/반도체/제주반도체/tech/jeju_semicon_tech_patent_evidence.json` | 확인됨 |
| ml_signal | `data/반도체/제주반도체/tech/tech_ml_signal.json` | 확인됨 |
...
| tech_ip_ml | `data/반도체/제주반도체/tech/tech_ip_ml.json` | 확인됨 |
