# SFA반도체 Tech Agent Full Appendix

- **생성 시각:** 2026-05-26T02:47:19
...
- **산출물 상태:** 확인됨: `data/반도체/SFA반도체/tech/sfa_semicon_tech_high_quality_report.md`
- **의미:** 기존 Excel 틀(high_quality_report.py)의 대분류를 Tech Agent 소유 풀버전 부록으로 옮긴 영역입니다.

| 대분류 | 항목 | 정량화 | 핵심 근거 | 등급 | 비고 |
|---|---|---:|---|---|---|
| 대표 기술 | 핵심 기술 키워드 | 4 / 5 (기술성) | SFA반도체은 Universe CSV에서 '후공정/패키징/테스트' peer_group으로 분류되어 있습니다.; 직접근거 3건, 공식/DART/IR/특허 근거 0건... | 강 | data\반도체\SFA반도체\tech\source\sfa_semicon_tech_local_proxy_evi... |
| 핵심 제품/서비스 | 제품명/서비스명 | 3 / 5 (수익성 기여) | SFA반도체은 Universe CSV에서 '후공정/패키징/테스트' peer_group으로 분류되어 있습니다.; 직접근거 6건, 공식/DART/IR/특허 근거 0건... | 중 | data\반도체\SFA반도체\tech\source\sfa_semicon_tech_local_proxy_evi... |
| 고객 구매 이유 | 고객 효익 | 3 / 5 (고객 채택도) | 핵심 원천 근거 추가 필요; 직접근거 0건, 공식/DART/IR/특허 근거 0건, 정량화 지표 9건 | 중 | - |
| 경쟁 우위/대체가능성 | 등록 특허 | 4 / 5 (진입장벽) | SFA반도체은 Universe CSV에서 '후공정/패키징/테스트' peer_group으로 분류되어 있습니다.; 직접근거 4건, 공식/DART/IR/특허 근거 0건... | 강 | data\반도체\SFA반도체\tech\source\sfa_semicon_tech_local_proxy_evi... |
| 활용 및 확장 산업 | 활용 산업 | 3 / 5 (확장성) | # SFA반도체 Tech Local Proxy Evidence; 직접근거 3건, 공식/DART/IR/특허 근거 0건, 정량화 지표 3건 | 중 | data\반도체\SFA반도체\tech\source\sfa_semicon_tech_local_proxy_evi... |
| 진입 부담/장벽 | 인증/승인 | 3 / 5 (양산성) | SFA반도체은 Universe CSV에서 '후공정/패키징/테스트' peer_group으로 분류되어 있습니다.; 직접근거 3건, 공식/DART/IR/특허 근거 0건... | 중 | data\반도체\SFA반도체\tech\source\sfa_semicon_tech_local_proxy_evi... |
| R&D 강도 | 연구개발비 | 3 / 5 (투자 지속성) | 핵심 원천 근거 추가 필요; 직접근거 0건, 공식/DART/IR/특허 근거 0건, 정량화 지표 6건 | 중 | - |
## Step 2. Tech-to-Value Bridge
- **산출물 상태:** 확인됨: `data/반도체/SFA반도체/tech/sfa_semicon_tech_to_value_inputs.json`
| 항목 | 값 | 해석 |
|---|---:|---|
...
- **산출물 상태:** 확인됨: `data/반도체/SFA반도체/tech/sfa_semicon_tech_patent_evidence.json`
| 원천 신호 | 값 |
|---|---:|
...
| 대표 특허 예시 | 상태/연도 |
|---|---|
| CIS 반도체 패키지의 제조방법 | 등록 |
| 차폐부 형성 방법 및 이에 따라 제조된 시스템 인 패키지 | 등록 |
| 웨이퍼 레벨의 팬 아웃 패키지 제조방법 | 등록 |
| 적층형 반도체 패키지 및 적층형 반도체 패키지의 제조방법 | 등록 |
| 팬-아웃형 웨이퍼 레벨 패키지의 제조 방법 | 등록 |
## Step 4. Patent KMeans clustering
- **산출물 상태:** 확인됨: `data/반도체/SFA반도체/tech/tech_ml_signal.json`
| 항목 | 값 |
|---|---:|
| 모델 | tfidf_kmeans |
| 정규화 특허 텍스트 레코드 | 30건 |
...
|---:|---:|---|---|
| 1 | 14건 | 반도체, 적층형 반도체, 패키지, 반도체 패키지, 적층형 | 기술 텍스트 유사도 기반 클러스터 |
| 0 | 12건 | 웨이퍼, 반도체, 제조 | 기술 텍스트 유사도 기반 클러스터 |
| 2 | 4건 | 접착, 반도체 접착, 장비, 접착 장비, 다이 접착, 다이, 반도체, 픽업 | 기술 텍스트 유사도 기반 클러스터 |
...
- **산출물 상태:** 확인됨: `data/반도체/SFA반도체/tech/tech_ip_ml.json`
### 9-1. Tech/IP Strength Index
| 항목 | 값 | 해석 |
|---|---:|---|
| Tech/IP Strength Index | 56.27/100 | 특허 등록률·존속성·최근성·IPC/CPC 다양성·핵심기술 키워드 밀도 점수를 통합한 IP 강도 지표입니다. |
| Reference Universe Percentile | 50.00% | reference universe 안에서의 상대적 기술/IP 위치입니다. |
| Patent Momentum Score | 89.68/100 | 최근 특허 활동과 기술 포트폴리오 지속성을 반영합니다. |
...
### 9-4. 해석
- sfa_semicon의 Tech/IP Strength Index는 56.27/100, percentile 50.0로 산출되어 중간 수준 IP 포지션으로 해석됩니다. NMF 기준 주된 기술 주제는 '주요 topic 확인 제한'입니다. 단, 이 지표는 기술/IP 포트폴리오 상대 강도 신호이며 고객 채택·양산·매출 전환·FCF 개선 근거를 대체하지 않습니다.
...
### 10-5. 해석
- SFA반도체의 Technology Differentiation Score는 peer 대비 기술/IP 포트폴리오 차별성 신호입니다.
...
| 구분 | 경로 | 상태 |
|---|---|---|
| chair_summary | `data/반도체/SFA반도체/tech/tech_chair_summary.json` | 확인됨 |
| tech_packet | `data/반도체/SFA반도체/tech/tech.json` | 확인됨 |
| high_quality_md | `data/반도체/SFA반도체/tech/sfa_semicon_tech_high_quality_report.md` | 확인됨 |
| tech_to_value | `data/반도체/SFA반도체/tech/sfa_semicon_tech_to_value_inputs.json` | 확인됨 |
| patent_json | `data/반도체/SFA반도체/tech/sfa_semicon_tech_patent_evidence.json` | 확인됨 |
| ml_signal | `data/반도체/SFA반도체/tech/tech_ml_signal.json` | 확인됨 |
...
| tech_ip_ml | `data/반도체/SFA반도체/tech/tech_ip_ml.json` | 확인됨 |
