# 티씨케이 Tech Agent Full Appendix

- **생성 시각:** 2026-05-26T02:18:31
...
- **산출물 상태:** 확인됨: `data/반도체/티씨케이/tech/tck_tech_high_quality_report.md`
- **의미:** 기존 Excel 틀(high_quality_report.py)의 대분류를 Tech Agent 소유 풀버전 부록으로 옮긴 영역입니다.

| 대분류 | 항목 | 정량화 | 핵심 근거 | 등급 | 비고 |
|---|---|---:|---|---|---|
| 대표 기술 | 핵심 기술 키워드 | 4 / 5 (기술성) | 티씨케이은 Universe CSV에서 '소재/부품' peer_group으로 분류되어 있습니다.; 직접근거 3건, 공식/DART/IR/특허 근거 0건, 정량화 지표... | 강 | data\반도체\티씨케이\tech\source\tck_tech_local_proxy_evidence.md |
...
| 활용 및 확장 산업 | 활용 산업 | 3 / 5 (확장성) | # 티씨케이 Tech Local Proxy Evidence; 직접근거 3건, 공식/DART/IR/특허 근거 0건, 정량화 지표 3건 | 중 | data\반도체\티씨케이\tech\source\tck_tech_local_proxy_evidence.md |
...
- **산출물 상태:** 확인됨: `data/반도체/티씨케이/tech/tck_tech_to_value_inputs.json`
| 항목 | 값 | 해석 |
|---|---:|---|
...
- **산출물 상태:** 확인됨: `data/반도체/티씨케이/tech/tck_tech_patent_evidence.json`
| 원천 신호 | 값 |
|---|---:|
...
| 대표 특허 예시 | 상태/연도 |
|---|---|
| 층간 경계를 덮는 증착층을 포함하는 반도체 제조용 부품 및 그 제조방법 | 등록 |
| 슬러리 막힘 방지 구조를 갖는 CMP용 멀티노즐 | 등록 |
| 반도체 제조용 부품의 재생 방법 및 재생된 반도체 제조용 부품 | 등록 |
| 투과도가 다른 복수 개의 층을 갖는 SiC 반도체 제조용 부품 및 그 제조방법 | 등록 |
| 반도체 제조용 부품의 재생 방법 및 재생된 반도체 제조용 부품 | 등록 |
## Step 4. Patent KMeans clustering
- **산출물 상태:** 확인됨: `data/반도체/티씨케이/tech/tech_ml_signal.json`
| 항목 | 값 |
|---|---:|
| 모델 | tfidf_kmeans |
| 정규화 특허 텍스트 레코드 | 30건 |
...
|---:|---:|---|---|
| 1 | 12건 | 반도체, 제조용, 반도체 제조용, 다른, 층을, sic, 갖는 | 기술 텍스트 유사도 기반 클러스터 |
| 2 | 10건 | 실리콘, sic, 실리콘 카바이드, 카바이드, 잉곳의, 단계와 | 기술 텍스트 유사도 기반 클러스터 |
| 0 | 8건 | 세라믹, 탄화붕소, 실리콘 | 기술 텍스트 유사도 기반 클러스터 |
...
- **산출물 상태:** 확인됨: `data/반도체/티씨케이/tech/tech_ip_ml.json`
### 9-1. Tech/IP Strength Index
| 항목 | 값 | 해석 |
|---|---:|---|
| Tech/IP Strength Index | 57.20/100 | 특허 등록률·존속성·최근성·IPC/CPC 다양성·핵심기술 키워드 밀도 점수를 통합한 IP 강도 지표입니다. |
| Reference Universe Percentile | 50.00% | reference universe 안에서의 상대적 기술/IP 위치입니다. |
| Patent Momentum Score | 84.03/100 | 최근 특허 활동과 기술 포트폴리오 지속성을 반영합니다. |
...
### 9-4. 해석
- tck의 Tech/IP Strength Index는 57.2/100, percentile 50.0로 산출되어 중간 수준 IP 포지션으로 해석됩니다. NMF 기준 주된 기술 주제는 '주요 topic 확인 제한'입니다. 단, 이 지표는 기술/IP 포트폴리오 상대 강도 신호이며 고객 채택·양산·매출 전환·FCF 개선 근거를 대체하지 않습니다.
...
### 10-5. 해석
- 티씨케이의 Technology Differentiation Score는 peer 대비 기술/IP 포트폴리오 차별성 신호입니다.
...
| 구분 | 경로 | 상태 |
|---|---|---|
| chair_summary | `data/반도체/티씨케이/tech/tech_chair_summary.json` | 확인됨 |
| tech_packet | `data/반도체/티씨케이/tech/tech.json` | 확인됨 |
| high_quality_md | `data/반도체/티씨케이/tech/tck_tech_high_quality_report.md` | 확인됨 |
| tech_to_value | `data/반도체/티씨케이/tech/tck_tech_to_value_inputs.json` | 확인됨 |
| patent_json | `data/반도체/티씨케이/tech/tck_tech_patent_evidence.json` | 확인됨 |
| ml_signal | `data/반도체/티씨케이/tech/tech_ml_signal.json` | 확인됨 |
...
| tech_ip_ml | `data/반도체/티씨케이/tech/tech_ip_ml.json` | 확인됨 |
