# GST Tech Agent Full Appendix

- **생성 시각:** 2026-05-26T06:32:41
...
- **산출물 상태:** 확인됨: `data/반도체/GST/tech/gst_tech_high_quality_report.md`
- **의미:** 기존 Excel 틀(high_quality_report.py)의 대분류를 Tech Agent 소유 풀버전 부록으로 옮긴 영역입니다.

| 대분류 | 항목 | 정량화 | 핵심 근거 | 등급 | 비고 |
|---|---|---:|---|---|---|
| 대표 기술 | 핵심 기술 키워드 | 3 / 5 (기술성) | 핵심 원천 근거 추가 필요; 직접근거 0건, 공식/DART/IR/특허 근거 0건, 정량화 지표 9건 | 중 | - |
| 핵심 제품/서비스 | 제품명/서비스명 | 3 / 5 (수익성 기여) | 핵심 원천 근거 추가 필요; 직접근거 0건, 공식/DART/IR/특허 근거 0건, 정량화 지표 6건 | 중 | - |
...
| 활용 및 확장 산업 | 활용 산업 | 3 / 5 (확장성) | # GST Tech Local Proxy Evidence; 직접근거 3건, 공식/DART/IR/특허 근거 0건, 정량화 지표 3건 | 중 | data\반도체\GST\tech\source\gst_tech_local_proxy_evidence.md |
...
- **산출물 상태:** 확인됨: `data/반도체/GST/tech/gst_tech_to_value_inputs.json`
| 항목 | 값 | 해석 |
|---|---:|---|
...
- **산출물 상태:** 확인됨: `data/반도체/GST/tech/gst_tech_patent_evidence.json`
| 원천 신호 | 값 |
|---|---:|
...
| 대표 특허 예시 | 상태/연도 |
|---|---|
| 회전드럼 내에서 부순모래가 파쇄 분리되면서 회전드럼에 일체로 형성된 미분분리실에서 에어폭기를 통해 연속적으로 미분이 분리되는 미분 분리장치 | 등록 |
| 전자부품용 핀프레임 절단장치 | 등록 |
| 조절 가능한 셀 비트 형상을 가지는 비휘발성 메모리 | 소멸 |
| 전기 도금된 상 변화 스위치 | 소멸 |
| 자기 기록 매체 및 그 제조 방법 | 소멸 |
## Step 4. Patent KMeans clustering
- **산출물 상태:** 확인됨: `data/반도체/GST/tech/tech_ml_signal.json`
| 항목 | 값 |
|---|---:|
| 모델 | tfidf_kmeans |
| 정규화 특허 텍스트 레코드 | 30건 |
...
|---:|---:|---|---|
| 2 | 14건 | 자기, 기록, 자성, 결정, 디스크, 자기 기록 | 기술 텍스트 유사도 기반 클러스터 |
| 0 | 8건 | 자기, 강자성, 반강자성, 쓰기, afc, 기록, smr, 자기 기록 | 기술 텍스트 유사도 기반 클러스터 |
| 1 | 8건 | 메모리, 엔진, 전극, 비트 | 기술 텍스트 유사도 기반 클러스터 |
...
- **산출물 상태:** 확인됨: `data/반도체/GST/tech/tech_ip_ml.json`
### 9-1. Tech/IP Strength Index
| 항목 | 값 | 해석 |
|---|---:|---|
| Tech/IP Strength Index | 50.83/100 | 특허 등록률·존속성·최근성·IPC/CPC 다양성·핵심기술 키워드 밀도 점수를 통합한 IP 강도 지표입니다. |
| Reference Universe Percentile | 50.00% | reference universe 안에서의 상대적 기술/IP 위치입니다. |
| Patent Momentum Score | 79.24/100 | 최근 특허 활동과 기술 포트폴리오 지속성을 반영합니다. |
...
### 9-4. 해석
- gst의 Tech/IP Strength Index는 50.83/100, percentile 50.0로 산출되어 중간 수준 IP 포지션으로 해석됩니다. NMF 기준 주된 기술 주제는 '주요 topic 확인 제한'입니다. 단, 이 지표는 기술/IP 포트폴리오 상대 강도 신호이며 고객 채택·양산·매출 전환·FCF 개선 근거를 대체하지 않습니다.
...
### 10-5. 해석
- GST의 Technology Differentiation Score는 peer 대비 기술/IP 포트폴리오 차별성 신호입니다.
...
| 구분 | 경로 | 상태 |
|---|---|---|
| chair_summary | `data/반도체/GST/tech/tech_chair_summary.json` | 확인됨 |
| tech_packet | `data/반도체/GST/tech/tech.json` | 확인됨 |
| high_quality_md | `data/반도체/GST/tech/gst_tech_high_quality_report.md` | 확인됨 |
| tech_to_value | `data/반도체/GST/tech/gst_tech_to_value_inputs.json` | 확인됨 |
| patent_json | `data/반도체/GST/tech/gst_tech_patent_evidence.json` | 확인됨 |
| ml_signal | `data/반도체/GST/tech/tech_ml_signal.json` | 확인됨 |
...
| tech_ip_ml | `data/반도체/GST/tech/tech_ip_ml.json` | 확인됨 |
