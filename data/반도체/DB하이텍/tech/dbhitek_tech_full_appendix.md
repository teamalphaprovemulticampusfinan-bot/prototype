# DB하이텍 Tech Agent Full Appendix

- **생성 시각:** 2026-05-20T21:16:38
...
- **산출물 상태:** 확인됨: `data/반도체/DB하이텍/tech/dbhitek_tech_high_quality_report.md`
- **의미:** 기존 Excel 틀(high_quality_report.py)의 대분류를 Tech Agent 소유 풀버전 부록으로 옮긴 영역입니다.

| 대분류 | 항목 | 정량화 | 핵심 근거 | 등급 | 비고 |
|---|---|---:|---|---|---|
| 대표 기술 | 핵심 기술 키워드 | 3 / 5 (기술성) | 핵심 원천 근거 추가 필요; 직접근거 0건, 공식/DART/IR/특허 근거 0건, 정량화 지표 6건 | 중 | - |
| 핵심 제품/서비스 | 제품명/서비스명 | 2 / 5 (수익성 기여) | 핵심 원천 근거 추가 필요; 직접근거 0건, 공식/DART/IR/특허 근거 0건, 정량화 지표 3건 | 보강 필요 | - |
| 고객 구매 이유 | 고객 효익 | 3 / 5 (고객 채택도) | 1020080062672; 직접근거 0건, 공식/DART/IR/특허 근거 0건, 정량화 지표 6건 | 중 | 1020100002694 |
| 경쟁 우위/대체가능성 | 등록 특허 | 4 / 5 (진입장벽) | 핵심 원천 근거 추가 필요; 직접근거 2건, 공식/DART/IR/특허 근거 1건, 정량화 지표 12건 | 강 | - |
| 활용 및 확장 산업 | 활용 산업 | 3 / 5 (확장성) | # DB하이텍 Tech Local Proxy Evidence; 직접근거 3건, 공식/DART/IR/특허 근거 0건, 정량화 지표 3건 | 중 | data\반도체\DB하이텍\tech\source\dbhitek_tech_local_proxy_evidence... |
| 진입 부담/장벽 | 인증/승인 | 3 / 5 (양산성) | 1020080062672; 직접근거 0건, 공식/DART/IR/특허 근거 0건, 정량화 지표 6건 | 중 | 1020100002694 |
| R&D 강도 | 연구개발비 | 3 / 5 (투자 지속성) | 1020080062672; 직접근거 0건, 공식/DART/IR/특허 근거 0건, 정량화 지표 6건 | 중 | 1020100002694 |
## Step 2. Tech-to-Value Bridge
- **산출물 상태:** 확인됨: `data/반도체/DB하이텍/tech/dbhitek_tech_to_value_inputs.json`
| 항목 | 값 | 해석 |
|---|---:|---|
...
- **산출물 상태:** 확인됨: `data/반도체/DB하이텍/tech/dbhitek_tech_patent_evidence.json`
| 원천 신호 | 값 |
|---|---:|
...
| 알에프 스위치 소자 | 등록 |
| 반도체 소자 및 제조방법 | 등록 |
| 고전압 반도체 소자 및 제조방법 | 공개 |
| 다마신 배선을 갖는 반도체 소자의 층간 절연막 제조 방법 | 소멸 |
## Step 4. Patent KMeans clustering
- **산출물 상태:** 확인됨: `data/반도체/DB하이텍/tech/tech_ml_signal.json`
| 항목 | 값 |
|---|---:|
| 모델 | tfidf_kmeans |
| 정규화 특허 텍스트 레코드 | 269건 |
...
|---:|---:|---|---|
| 4 | 91건 | 웨이퍼, 웨이퍼의, 반도체, 금속 | 기술 텍스트 유사도 기반 클러스터 |
| 6 | 44건 | 반도체 소자의, 소자의, 반도체, 단계와, 마스크 | 기술 텍스트 유사도 기반 클러스터 |
| 7 | 36건 | 게이트, 반도체, 단계와, 메모리 | 기술 텍스트 유사도 기반 클러스터 |
| 1 | 27건 | 이미지, 패턴을 | 기술 텍스트 유사도 기반 클러스터 |
| 0 | 26건 | 도전형의, 영역, 영역과, 반도체 | 기술 텍스트 유사도 기반 클러스터 |
| 2 | 18건 | 이미지, 이미지 센서, 센서, 배치된, 포토, 상에 배치된 | 기술 텍스트 유사도 기반 클러스터 |
| 5 | 15건 | 반도체 소자, 소자, 소자 제조방법에, 고전압 반도체, 고전압, 제조방법에 관한, 제조방법에 | 기술 텍스트 유사도 기반 클러스터 |
| 3 | 12건 | 형성된, 이미지센서, 리드아웃, 따른 이미지센서는, 이미지센서는, 상에 형성된 | 기술 텍스트 유사도 기반 클러스터 |
...
- **산출물 상태:** 확인됨: `data/반도체/DB하이텍/tech/tech_ip_ml.json`
### 9-1. Tech/IP Strength Index
| 항목 | 값 | 해석 |
|---|---:|---|
| Tech/IP Strength Index | 44.82/100 | 특허 등록률·존속성·최근성·IPC/CPC 다양성·핵심기술 키워드 밀도 점수를 통합한 IP 강도 지표입니다. |
| Reference Universe Percentile | 50.00% | reference universe 안에서의 상대적 기술/IP 위치입니다. |
| Patent Momentum Score | 0.58/100 | 최근 특허 활동과 기술 포트폴리오 지속성을 반영합니다. |
...
### 9-4. 해석
- dbhitek의 Tech/IP Strength Index는 44.82/100, percentile 50.0로 산출되어 근거 보강 필요 IP 포지션으로 해석됩니다. NMF 기준 주된 기술 주제는 '주요 topic 확인 제한'입니다. 단, 이 지표는 기술/IP 포트폴리오 상대 강도 신호이며 고객 채택·양산·매출 전환·FCF 개선 근거를 대체하지 않습니다.
...
### 10-5. 해석
- DB하이텍의 Technology Differentiation Score는 peer 대비 기술/IP 포트폴리오 차별성 신호입니다.
...
| 구분 | 경로 | 상태 |
|---|---|---|
| chair_summary | `data/반도체/DB하이텍/tech/tech_chair_summary.json` | 확인됨 |
| tech_packet | `data/반도체/DB하이텍/tech/tech.json` | 확인됨 |
| high_quality_md | `data/반도체/DB하이텍/tech/dbhitek_tech_high_quality_report.md` | 확인됨 |
| tech_to_value | `data/반도체/DB하이텍/tech/dbhitek_tech_to_value_inputs.json` | 확인됨 |
| patent_json | `data/반도체/DB하이텍/tech/dbhitek_tech_patent_evidence.json` | 확인됨 |
| ml_signal | `data/반도체/DB하이텍/tech/tech_ml_signal.json` | 확인됨 |
...
| tech_ip_ml | `data/반도체/DB하이텍/tech/tech_ip_ml.json` | 확인됨 |
